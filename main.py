...
import matplotlib.pyplot as plt #to visualize data graph
...
new_df = new_df.drop(columns=['UNITID', 'OPEID', 'opeid6'])  # drop unnecessary columns zy el id
...
# 1.new_df.fillna(method='bfill', inplace=True)  # fill with next values
# new_df.fillna(method='ffill', inplace=True)  # fill with previous values
# 2.
...
for col in new_df.columns:
    if new_df[col].dtype == 'O':  # convert string columns to be able to apply feature selection & models
        new_df[col] = encode.fit_transform(new_df[col])#y read enhom b integer khlas
...
plt.show()
#vt fit kany b3ml mnha train and predict(more effiecent in unsupervised learning) eny kda b3ml test 
print("3rd Birch:")
...
print("6th Gaussian Mixture:")
gauss = GaussianMixture(n_components=3).fit_predict(VT)  # better than kmeans as it model non_circular data too not only circular ones
print("Labels:", gauss)
labels = np.unique(gauss)
for i in labels:
    plt.scatter(VT[gauss == i, 0], VT[gauss == i, 1], label=i)
plt.title("Gaussian Mixture Diagram")
plt.show()
